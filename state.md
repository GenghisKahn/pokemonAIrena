...
> loss.** The KB is complete (165 Gen 1 moves + 151 species). The vision path also runs on **Windows**
> now (PrintWindow capture + Tesseract OCR + `SendInput`, merged from PR #1 and reconciled onto the macOS
> work on this `combined_dev` branch). The one remaining step is running `python app.py` so the *agent*
> drives a battle end to end, then tuning timings. See HANDOFF.md for the input model, emulator config,
> the Windows-support notes, and next steps.
